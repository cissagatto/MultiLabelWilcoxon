...
#'     \item \code{type}: An integer vector representing the type of 
#'     each metric.
#'   }
...
#'   \item \code{0}: Metrics that are less commonly used or have different 
#'   evaluation criteria.
#' }
#' 
#' @examples
#' # Create the data frame of metrics
#' metric_df <- w.measures()
#' 
...
#'
#' This function extracts measure names from a vector of file paths by removing the file extensions.
#' The measure names are derived from the file names of the provided file paths.
#'
#' @param file_paths A character vector of file paths.
#' @return A character vector containing the measure names extracted from the file names.
#' @examples
#' file_paths <- c("/path/to/data/accuracy.csv", "/path/to/data/precision.csv")
#' extract_measure_names(file_paths)
#' # Returns: "accuracy" "precision"
#' @export
